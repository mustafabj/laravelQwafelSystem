            ...
            <div class="table-container">
                <table class="data-table" id="parcelsTable">
                    ...
                    <tbody id="indexParecelsBody">
                        ...
                        @php
                            $parcelForModal = $parcel->loadMissing(['customer', 'originOffice', 'destinationOffice', 'details', 'user']);
                            $parcelModalHtml = view('Parcels.show', ['parcel' => $parcelForModal, 'user' => auth()->user()])->render();
                            // Encode with proper UTF-8 handling for JavaScript decodeURIComponent
                            $parcelModalEncoded = base64_encode($parcelModalHtml);
                        @endphp
                        <tr data-parcel-id="{{ $parcel->parcelId }}"
                            data-parcel-modal="{{ $parcelModalEncoded }}"
            ...
            <div class="table-container">
                <table class="data-table" id="indexTicketTable">
                    ...
                    <tbody id="indexTicketBody">
                        ...
                        @php
                            $ticketForModal = $ticket->loadMissing(['customer', 'address', 'office', 'user']);
                            $currencyMap = [
                                'JD' => 'دينار',
                                'USD' => 'دولار أمريكي',
                                'IQD' => 'دينار عراقي',
                            ...
                            $ticket->unpaid_amount = $ticket->paid !== 'paid' ? ($ticket->cost - $ticket->costRest) : 0;
                            ...
                            $ticketModalHtml = view('Tickets.show', compact('ticket'))->render();
                            // Encode with proper UTF-8 handling for JavaScript decodeURIComponent
                            $ticketModalEncoded = base64_encode($ticketModalHtml);
                        @endphp
                        <tr data-ticket-id="{{ $ticket->ticketId }}"
                            data-ticket-modal="{{ $ticketModalEncoded }}"
            ...
            <div class="table-container">
                <table class="data-table" id="driverParcelsTable">
                    ...
                    <tbody id="driverParcelsBody">
                        ...
                        @php
                            $driverParcel->loadMissing(['trip', 'office', 'user', 'details.parcelDetail.parcel.customer']);
                            $driverParcelModalHtml = view('DriverParcels.show', ['driverParcel' => $driverParcel])->render();
                            $driverParcelModalEncoded = base64_encode($driverParcelModalHtml);
                        @endphp
                        <tr data-driver-parcel-id="{{ $driverParcel->parcelId }}"
                            data-driver-parcel-modal="{{ $driverParcelModalEncoded }}"
